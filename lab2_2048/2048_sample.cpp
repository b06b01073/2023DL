...
class board {
public:
	board(uint64_t raw = 0) : raw(raw) {} // each entry takes 4 bits, the value of the original board can be up to 2^(31)
	 ...
	 * get a 4-bit tile
	 */
	int at(int i) const { return (raw >> (i << 2)) & 0x0f; } // i start from the last position of `raw`
	/**
	 * set a 4-bit tile
	 */
	void set(int i, int t) { raw = (raw & ~(0x0fULL << (i << 2))) | (uint64_t(t & 0x0f) << (i << 2)); } // t is a power of 2
...
public:
	...
	/**
	 * apply an action to the board, the board is modified by modifying the raw data
...
public: // should be implemented, the implementation should be in the child class `pattern`
...
protected:
	static float* alloc(size_t num) {
		...
		try {
			...
			return new float[num](); //() initialize all entries to 0
		} catch (std::bad_alloc&) {
...
public:
	// the feature is initialized here, we can now use weights defined in the feature class, ex: pattern({0, 1, 2, 3}), the len of feature will be 1 << 16 = 2^16 = 4 ^ 4. It initialize a weight array(float) with a size of 2^16(p.size() * 4因為共p.size()個格子，每個格子需要4bits) 
	pattern(const std::vector<int>& p, int iso = 8) : feature(1 << (p.size() * 4)), iso_last(iso) {
		...
		for (int i = 0; i < 8; i++) {
			...
			if (i >= 4) idx.mirror();
			idx.rotate(i);
			// p are the positions on the board which the pattern takes care of
			for (int t : p) {
				isomorphic[i].push_back(idx.at(t)); // isomorphic[i]涵蓋pattern所捕捉的位置
			}
		}
	}
...
public:
	...
	virtual float update(const board& b, float u) {
		/* 
		TODO, u is probably the learning rate, call b.popup() to get s'_next?
		*/ 
		u /= iso_last; // note that division here is a must, since in the estimate function sums up all isomorphic estimation. Without the division, the estimate will give incorrect result.
...
protected:
	...
	size_t indexof(const std::vector<int>& patt, const board& b) const {
		// TODO, given a pattern and the board position, what is the index of that position in the weight table
		size_t index = 0; 
		for(int i = 0; i < patt.size(); i++) {
			index |= b.at(patt[i]); //"endianess" doesn't matter here as long as every set and get operation from the weight is consistent

			if(i != patt.size() - 1)
				index <<= 4; // "go for the next square"
		}
		return index;
	}
	...
	std::array<std::vector<int>, 8> isomorphic; // 8 isomorphism
	int iso_last;
};
...
public:
	...
	/** seems like this is for state transition
	...
	bool assign(const board& b) {
		...
		esti = score; // immediate reward here
		return score != -1;
	}
...
public:
	...
	void add_feature(feature* feat) {
		feats.push_back(feat); // private: std::vector<feature*>
	 ...
	 * It checks every features(n-tuple) and sums up the estimated value 
	 */
	...
	/**
	 * update the value of given state and return its new value, why return the new value?
	 */
	float update(const board& b, float u) const {
		debug << "update " << " (" << u << ")" << std::endl << b; 
		float u_split = u / feats.size();  // Learning rate for features of 𝑛-tuple network with 𝑚 features: 0.1 ÷ 𝑚
		float value = 0;
		for (feature* feat : feats) {
			value += feat->update(b, u_split); // it is implemented in the update function of pattern class
		}
		return value;
	}
	 ...
	 *  before_state() is b, done in assign
	 *  after_state() is b's best successor (after state), done in move function in assign function
	 *  action() is the best action, set in the constructor
	 *  reward() is the reward of performing action(), done in assign
	 *  value() is the estimated value of after_state(), done in the TODO
	 *
	 * you may simply return state() if no valid move
	 */
	state select_best_move(const board& b) const {
		state after[4] = { 0, 1, 2, 3 }; // up, right, down, left, read the constructor, the number represents the opcode 
		state* best = after;
		for (state* move = after; move != after + 4; move++) {
			//(move->assign(b) returns a boolean, it is true if the action is legal)
			if (move->assign(b)) {
				// TODO, calculate the move->value() by considering every possible next state
		...
		// return the best move based on the condition above
		return *best;
	}
	 ...
	 * 
	 * (s0,s0',a0,r0) = (state.before_state(), state.after_state(), state.action(), state.reward())
	 */
	void update_episode(std::vector<state>& path, float alpha = 0.1) const {
		// TODO
		// probably should call the update function in public functions
		// probably should iterate over every states:
		// each state should be used to update the pattern in features(its actually the pattern class instances) 
		// backward update
...
int main(int argc, const char* argv[]) {
	...
	// initialize the features, pattern is upcasted to be treated as a feature class instance
	tdl.add_feature(new pattern({ 0, 1, 2, 3, 4, 5 }));
	tdl.add_feature(new pattern({ 4, 5, 6, 7, 8, 9 }));
	tdl.add_feature(new pattern({ 0, 1, 2, 4, 5, 6 })); //rec
	tdl.add_feature(new pattern({ 4, 5, 6, 8, 9, 10 })); //rec

	// restore the model from file
	tdl.load("best.bin");

	// train the model
	std::vector<state> path; // save the history of the entire episode
	path.reserve(20000);

	// play the game `total` times
	for (size_t n = 1; n <= total; n++) {
		...
		while (true) {
			...
			state best = tdl.select_best_move(b); // select the best move and play
			path.push_back(best); // (s0,s0',a0,r0) = (best.before_state(), best.after_state(), best.action(), best.reward())

			// check whether the selected move is legal
			if (best.is_valid()) {
				...
				// invalid move => lower the learning rate
				break;
			}
		}
